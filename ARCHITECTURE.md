...
3. **One call per intent (one-shot), or a supervised session.** `dispatch` = (ensure daemon → start or reattach → wait → print result) and `reply` = (follow → wait → print last-turn result) are symmetric single background calls. A bounded `dispatch --timeout` keeps the door to `status`/`steer` open mid-run: it returns exit `1` with the worker still running, and repeating the call reattaches. The orchestrator selects either interface by whether supervision can change the outcome, without a fixed cadence.
4. **Mode is harness policy, not memory.** `dispatch` requires
