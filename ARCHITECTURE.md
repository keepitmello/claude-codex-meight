...
3. **One call per intent.** `dispatch` = (ensure daemon → start → wait → print result). `reply` = (follow → wait → print last-turn result). Symmetric, single background call each, so driving a Codex worker costs the orchestrator the same number of tool calls as a native subagent.
