...
3. **One call per intent (one-shot), or a supervised loop.** `dispatch` = (ensure daemon → start → wait → print result) and `reply` = (follow → wait → print last-turn result) are symmetric single background calls — one-shot driving costs the same tool calls as a native subagent, which suits trivial work. For substantial work the default is supervised: `start` once, then a periodic background `wait --timeout` as a checkpoint, reading `status` and issuing `steer` between checkpoints. Same pull/push primitives — just sampled more than once instead of a single fire-and-forget.
4. **Two-way by protocol, not plumbing.** A preamble (auto-prepended to every brief) tells workers: never commit, and when blocked end your reply with a `QUESTION:` paragraph instead of guessing. The daemon promotes that to `needs_input` → exit 3 → the orchestrator answers with `reply` on the same thread.
...
| High-stakes paths (e.g. anything touching money) | Either — but runtime evidence + explicit orchestrator sign-off regardless |
