...
3. **Direction-setting forks get two reads, never one.** The silent failure is deciding a branch that sets direction — which approach, a design tradeoff, an architecture or diagnosis call, scope/sequencing — *alone*: analyzing it yourself and committing without ever calling a worker. On such forks you analyze first (analysis is never outsourced), then run a mandatory independent read-only Codex analysis of the same question (the consult pattern above, `--sandbox ro`), and set direction by comparing the two reads. Not calling Codex on a direction fork is the same failure as skipping cross-model review — your reasoning alone is a claim, the cross-read makes it a decision. Trivial or already-agreed calls are exempt.
