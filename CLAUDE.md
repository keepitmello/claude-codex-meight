...
2. **Independent review before accepting is mandatory — the form is your call by risk.** Every worker output gets an independent read; the implementer never reviews its own work (that's the one non-negotiable). Choose the form: **cross-model** (Codex implements → fresh Claude agent verifies; Claude implements → Codex `--sandbox ro --effort high` worker reviews) adds model diversity — default for subtle/high-stakes/irreversible work (money-path, concurrency, auth, migration, external APIs); **fresh Codex review worker** (a *separate* worker from the implementer, adversarial `--sandbox ro`) is same-model but independent context — fine for routine, lower-risk, well-specced changes, and cheaper than a Claude agent. Either way demand P1/P2/P3 + file:line + GO/NO-GO, re-review via `follow` on the *review* worker. When unsure, prefer cross-model. What's banned is accepting "done" with no independent read.
