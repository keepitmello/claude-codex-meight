...
- Codex worker가 소유: 참여형 bounded 기술 설계, 구현, 검증, 로컬 실행 선택. 외부 리뷰의 선택과 처리는 디스패처가 소유한다.
- delegate가 소유: 구현과 내부 리뷰 end to end. 디스패처는 기술 컨텍스트 밖에 머문다.
- mate가 소유: 플랜·방향·코드·독트린에 대한 독립적 반론.

mate / worker / delegate는 모델이 아니라 세션 계약의 이름이다 — `--mode`가 계약을 고르고 `--model`이 두뇌를 고른다. 셋 다 침묵 실행자가 아니라 팀원 계약이라, 더 나은 방향·잘못된 전제·자기 소유 밖 결정이 보이면 구조화된 `QUESTION:`으로 밀어낼 수 있다.

검증이 결과를 소유한다. 워커의 "done"은 주장이고, 관련 테스트나 런타임 확인 + 디스패처 sign-off가 그걸 사실로 만든다. 리뷰가 값을 한다고 디스패처가 판단한 작업에서는 sign-off = 리뷰 verdict + 검증 증거다. 디스패처는 자기 게이트 선택을 한 줄로 기록한다. 기본 전달 체인 같은 건 없다.
