...
스코프가 겹치지 않는 워크스트림들이 보이면 줄 세우지 말고 한꺼번에 띄운다 — 세션마다 별도 구독이라 동시성은 이미 쥔 공짜 용량이고, 순차로 흘리는 게 그걸 버리는 쪽이다. 반대로 같은 워크스트림의 다음 단계는 새 dispatch가 아니라 `follow`다 — thread가 레포 컨텍스트와 프롬프트 캐시를 그대로 이어받아, 새 세션이 콜드로 다시 태울 탐색 토큰을 아낀다. 경계는 크기가 아니라 스코프다: 순차 의존인 단계를 세션 여러 개로 나누면 동시성 없이 콜드 스타트 비용만 곱하고, 별개 스코프를 한 세션에 줄 세우면 병렬 이득을 버린다. 앵커링이 적인 판정 자리(리뷰·블라인드 설계)만은 언제나 fresh 세션이다.
