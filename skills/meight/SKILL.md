...
- **Two modes — pick one up front, ask the user if unsure**: the harness runs in two modes that differ by *who owns the technical direction*. **Collaborative** — you own technical judgment too and cross-read approach/design with workers ("Direction is set by two reads", Consult). **Delegated** — you own only *what/why* (a product role); the worker owns all of *how*, closes the technical loop itself, and reports only a dashboard (self-reviewing worker, "product out of the technical ping-pong"). Mixing them is the failure mode: surfacing technical options to a product-role caller for them to pick, or deciding a technical fork solo when you're the tech lead. If which mode this work is in isn't obvious, **ask the user before dispatching** rather than guessing.
...
A worker's `QUESTION:` isn't only "I'm blocked" — under the teammate preamble it's also how a worker flags a better approach, a shaky assumption, or a tradeoff that needs your call. Keep that channel narrow: `QUESTION:` is for product-owned decisions such as scope, UX/product behavior, priority, risk appetite, irreversible action, or acceptance-criteria conflict. Technical uncertainty should be resolved with evidence first; local implementation choices should be decided by the worker and recorded as judgment calls, not escalated as product questions. In supervised mode, read it with `meight status <name>` (`needs_input_detail`) or `meight result <name>`; in one-shot `dispatch`/`reply` it's in the printed result. Answer — or discuss back — in one shot:
...
- **Abstract the report**: detailed findings, per-round review logs, command output, and reasoning go to a worker-unique evidence artifact such as `<worker-name>-evidence.md`; the report body you receive is a fixed dashboard plus concise verification summary. (This is the concrete form of the guide's Planning Report Mode.)
- **Keep sign-off evidence on the surface**: the report body must include `VERIFICATION` lines with `PASS` / `FAIL` / `NOT RUN` plus one short evidence note for each spec check or verification scenario. This summary is product sign-off evidence, not technical log spill.
...
4. Report to product in exactly this shape, nothing else:
   VERDICT: GO / NO-GO        (does it meet the spec = the what/why I gave you)
   VERIFICATION: <each spec check / verification scenario: PASS / FAIL / NOT RUN + one short evidence line>
   P1 RESOLVED: <count> — <one title line each, no code / no technical explanation>
   NEEDS PRODUCT DECISION: <scope/UX/priority/product-judgment/tradeoff calls only, one line each; "none" if none>
   FILES: <changed files>
   COMMIT MSG: <one-line suggested commit message>
   DETAILS: see <worker-name>-evidence.md
```

When to use which: **self-reviewing worker** = bounded implementation where Codex should own technical execution and keep product out of technical detail. **Separate review worker / cross-model A/B** (above) = high-stakes or architecture work where you *want* to read the findings and weigh them yourself.
