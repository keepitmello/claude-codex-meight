...
같은 워크스트림의 다음 단계는 새 dispatch가 아니라 `follow`다 — thread가 레포 컨텍스트와 프롬프트 캐시를 그대로 이어받아, 새 세션이 콜드로 다시 태울 탐색 토큰을 아낀다. 새 세션은 두 자리에 연다: 파일 스코프가 겹치지 않고 실제로 동시에 도는 별개 워크스트림, 그리고 앵커링이 적인 판정 자리(리뷰·블라인드 설계). 병렬 dispatch는 그 첫 자리에만 해당한다 — 순차 의존인 단계를 세션 여러 개로 나누면 동시성 없이 콜드 스타트 비용만 곱한다.
