...
브리프에 담는 것은 **무엇을 가능하게 하려는가, 무엇을 만족해야 끝인가, 경계가 어디까지인가**다. how는 워커 소유라, 조사 항목·문서 목차·구현 순서를 미리 깔면 산출물은 나와도 발견이 사라진다. mode/QUESTION 정책은 preamble이 주입하니 도메인 규칙과 작업별 제약만 넣는다.
