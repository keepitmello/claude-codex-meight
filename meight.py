...
- When you write detailed `result.md` content, make it self-contained for a follow-up worker, not just an archive for yourself: include the actionable handoff (what the next worker should do), relevant file/line evidence, verification commands, and any open decisions — so the next worker can pick up from `result.md` alone and the PM never has to re-read or re-translate technical logs into a new brief.
- If you leave non-code artifact documents such as reports, analyses, or handoffs in the working directory (cwd), do not use fixed generic names like `result.md`; parallel workers in the same cwd can overwrite each other and pollute the repo. Use a worker-unique name such as `<worker-name>-<short-topic>.md`, and keep that worker-name prefix for every cwd artifact document you create. The isolated worker report at `~/.meight/repos/.../workers/<name>/result.md` remains the standard reporting channel because it is already separated per worker. Code changes should be made directly in their source paths and are not part of this artifact-document naming rule.
