...
동봉된 오퍼레이터 정책 템플릿은 bounded 작업을 `luna`로 보내고,
acceptance-critical한 동시성·보안·공개 API 계약 설계·데이터 마이그레이션·
cross-cutting 리팩터, 그리고 돈/데이터 손상이나 비가역 피해를 낳을 수 있는
작업에 실패 비용 하드 게이트를 건다. 이 모델·돈 경로 게이트는 명시적으로
